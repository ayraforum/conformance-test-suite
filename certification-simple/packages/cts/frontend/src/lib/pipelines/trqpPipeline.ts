...
export default class TRQPPipeline {
  _dag: DAG;
  _agent: BaseAgent;
  ...
  _make(agent: BaseAgent): DAG {
    const dag = new DAG('TRQP Pipeline');
    ...
    const requestTRQPProofTask = new RequestProofTask(
      agent,
      {
        ...requestProofOptions,
        checkTrustRegistry: true,
      ...
      'Request TRQP Proof',
      'Request a proof from the holder and verify against TRQP'
    );
    ...
    const trqpNode = new TaskNode(requestTRQPProofTask);
    trqpNode.addDependency(proofNode);
    dag.addNode(trqpNode);
