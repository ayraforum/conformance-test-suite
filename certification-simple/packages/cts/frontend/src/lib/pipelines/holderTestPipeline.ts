...
import { RequestProofOptions, SetupConnectionTask, RequestProofTask } from "../core/agent/tasks";
import { DAG } from "../core/pipeline/src/dag";
import { KeyType, TypedArrayEncoder } from "@credo-ts/core";

export class HolderTestPipeline {
  _dag?: DAG;
  _agent: BaseAgent;
  ...
  _make(agent: BaseAgent, did: string): DAG {
    const dag = new DAG("Holder Conformance Test Pipeline");
    ...
    // Step 2: Request Proof/Presentation
    ...
    proofRequestNode.addDependency(connectionNode);
